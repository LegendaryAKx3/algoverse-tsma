#!/usr/bin/env python
"""
Train linear probe for Monte Carlo equity prediction on preprocessed poker hands.

This script loads a preprocessed poker dataset and trains a PyTorch linear regression
...
class LinearRegressionProbe(nn.Module):
    """Simple linear regression probe using PyTorch."""
    ...
    def forward(self, x):
        return self.linear(x).squeeze(-1)  # Return shape (batch_size,)
...
# ---------------------------
def train_equity_probe(dataset_path, model_artifacts_path="artifacts", max_samples=None, 
                      epochs=1000, learning_rate=0.001, batch_size=64, huggingface_model=None):
    """
    Train linear regression probe on poker equity prediction.
    ...
    # Initialize PyTorch linear regression probe
    print("Training PyTorch linear regression probe...")
    input_dim = emb_train.shape[1]
    regressor = LinearRegressionProbe(input_dim).to(device)
    ...
    os.makedirs("confusion_matrices/equityRegression", exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig("confusion_matrices/equityRegression/equity_regression_plots.png", dpi=300)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig("confusion_matrices/equityRegression/equity_distribution.png", dpi=300)
    plt.close()
    ...
    torch.save({
        ...
        'input_dim': input_dim,
        'results': results
    }, "confusion_matrices/equityRegression/equity_probe_model.pth")
    
    with open("confusion_matrices/equityRegression/results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\nResults saved to confusion_matrices/equityRegression/")
    print("Plots: equity_regression_plots.png, equity_distribution.png")
    print("Model: equity_probe_model.pth")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train linear probe for poker equity prediction")
